...
{
	/* Shared L1: 43612 bytes, L2 buffer: 14000 bytes */
	...
	_N_Rec_Kernel2=0;
	AT_L2_COPY2D(0, ((AT_L2_EXT_ADDR_TYPE) Rec_Kernel2+0), ((AT_L2_INT_ADDR_TYPE) L1_Memory+15612+0), 7000, 500, 28, 0, &DmaR_Evt6);
	_N_Bias2=0;
	...
	{ /* Single iteration on Tile2 */
		...
		for(Iterazione=0;Iterazione<27;Iterazione++){	
			...
			int T1Ind_Last = (T1Ind==17), T1Ind_NextLast = ((T1Ind+1)==17);
			/*================================= Prepare Tiles ===================================*/
			_SN_Rec_Kernel2 = 0;
			if (!(T1Ind_Last)) {
				_N_Rec_Kernel2 = _N_Rec_Kernel2 + (28); _LN_Rec_Kernel2 = ((T1Ind_NextLast)?24:28); _SN_Rec_Kernel2 = (250*_LN_Rec_Kernel2); 
			} else {
				_N_Rec_Kernel2 = _N_Rec_Kernel2 + (-476); _LN_Rec_Kernel2 = (28); _SN_Rec_Kernel2 = (250*_LN_Rec_Kernel2); 
			}
			/*============================= End Prepare Tiles ===================================*/
			/*================================= Read Tiles ======================================*/
			AT_L2_WAIT(0, &DmaR_Evt6); /* Wait previous DMA read Rec_Kernel2 */
			if (_SN_Rec_Kernel2) {
				AT_L2_COPY2D(0, ((AT_L2_EXT_ADDR_TYPE) Rec_Kernel2+_N_Rec_Kernel2), ((AT_L2_INT_ADDR_TYPE) L1_Memory+15612+7000*((T1Ind_Total+1)%2)),
						_SN_Rec_Kernel2, 500, _LN_Rec_Kernel2, 0, &DmaR_Evt6);
			}			
...
{
	/* Shared L1: 43548 bytes, L2 buffer: 15000 bytes */
	/* Local variables used by this kernel */
	AT_L2_EVENT DmaR_Evt1;
	AT_L2_EVENT DmaR_Evt2;
	AT_L2_EVENT DmaR_Evt3;
	...
	_N_Rec_Kernel2=0;
	AT_L2_COPY2D(0, ((AT_L2_EXT_ADDR_TYPE) Rec_Kernel2+0), ((AT_L2_INT_ADDR_TYPE) L1_Memory+13548+0), 7500, 500, 30, 0, &DmaR_Evt3);
	_N_Bias2=0;
	...
	for(Iterazione=0;Iterazione<9;Iterazione++){
		...
		for (T1Ind=0; T1Ind<17; T1Ind++, T1Ind_Total++) { 
			int T1Ind_Last = (T1Ind==16), T1Ind_NextLast = ((T1Ind+1)==16);

			_SN_Rec_Kernel2 = 0;
			if (!(T1Ind_Last)) {
				_N_Rec_Kernel2 = _N_Rec_Kernel2 + (30); _LN_Rec_Kernel2 = ((T1Ind_NextLast)?20:30); _SN_Rec_Kernel2 = (250*_LN_Rec_Kernel2); 
			} else  {
				_N_Rec_Kernel2 = _N_Rec_Kernel2 + (-480); _LN_Rec_Kernel2 = (30); _SN_Rec_Kernel2 = (250*_LN_Rec_Kernel2); 
			}

			AT_L2_WAIT(0, &DmaR_Evt3); 
			if (_SN_Rec_Kernel2) {
				AT_L2_COPY2D(0, ((AT_L2_EXT_ADDR_TYPE) Rec_Kernel2+_N_Rec_Kernel2), ((AT_L2_INT_ADDR_TYPE) L1_Memory+13548+7500*((T1Ind_Total+1)%2)),
						_SN_Rec_Kernel2, 500, _LN_Rec_Kernel2, 0, &DmaR_Evt3);
			}
