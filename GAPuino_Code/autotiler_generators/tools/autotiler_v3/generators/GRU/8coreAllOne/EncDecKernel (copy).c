...
//calcolo h_tilde
	for (Col=First; Col<Last; Col++) {
			...
			if(CoreId==0&&Iterazione==0&&Col==0){printf("S [%d]=%d     Bias[%d]=%d    Kernel[%d]=%d\n",Col+OutFirstCol,S,index,Bias[index],index,Kernel[index]);}
            for (i=0; i<W_H/2; i++) {
				if(CoreId==0&&Iterazione==0&&Col==0){printf("prod1[%d]=%d     Rec_Kernel[%d]=%d\n",2*i,prod1[(2*i  )],(2*i  )*W_Rec_Kernel+Col,Rec_Kernel[(2*i  )*W_Rec_Kernel+Col]);}				
                S = S + prod1[(2*i  )] * Rec_Kernel[(2*i  )*W_Rec_Kernel+Col];
				if(CoreId==0&&Iterazione==0&&Col==0){printf("S [%d]=%d\n",Col+OutFirstCol,S);}
