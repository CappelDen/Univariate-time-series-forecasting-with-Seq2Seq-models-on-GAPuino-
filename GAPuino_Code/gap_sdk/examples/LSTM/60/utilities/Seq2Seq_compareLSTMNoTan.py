...
#carico il modello
model = h5py.File('27_9EDU1LSTM10T_150OneYearNoTan.h5', 'r')

#carico lo scaler
scaler = load(open('scaler.pkl', 'rb'))

#Leggo i pesi della rete
dense_bias = np.array((((model['model_weights'])['dense_3'])['dense_3'])['bias:0'])
dense_kernel = np.array((((model['model_weights'])['dense_3'])['dense_3'])['kernel:0'])

Encoder_lstm_bias = np.array((((model['model_weights'])['rnn_5'])['rnn_5'])['bias:0'])
Encoder_lstm_kernel = np.array((((model['model_weights'])['rnn_5'])['rnn_5'])['kernel:0'])
Encoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_5'])['rnn_5'])['recurrent_kernel:0'])

Decoder_lstm_bias = np.array((((model['model_weights'])['rnn_6'])['rnn_6'])['bias:0'])
Decoder_lstm_kernel = np.array((((model['model_weights'])['rnn_6'])['rnn_6'])['kernel:0'])
Decoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_6'])['rnn_6'])['recurrent_kernel:0'])
...
#	print(x[n_input,:])
	...
	#for n_steps in range(1):
		...
		#print(i_lstm)
		c_lstm = np.multiply(f_lstm, Encoder_c_0_lstm) + np.multiply(i_lstm, input_to_c_lstm)
		h_lstm = np.multiply(o_lstm, c_lstm)
...
#	print(Encoder_c_0_lstm)
	...
	#for n_steps in range(1):
		...
		c_lstmD = np.multiply(f_lstmD, Decoder_c_0_lstm) + np.multiply(i_lstmD, input_to_c_lstmD)
		h_lstmD = np.multiply(o_lstmD, c_lstmD)
...
#for n_input in range(1):
	...
	for n_steps in range(steps_in):
		...
		input_to_c_lstm_fix = z_lstm_fix[:, layers*2:layers*3]>>scale_shift#(32,24)		

		o_lstm_fix = uf.hard_sig(z_lstm_fix[:, layers*3:layers*4])#(32,12)
		c_lstm_fix = np.multiply(f_lstm_fix, Encoder_c_0_lstm_fix) + np.multiply(i_lstm_fix, input_to_c_lstm_fix)#(32,24)
		h_lstm_fix = np.multiply(o_lstm_fix, c_lstm_fix>>scale_shift)#(32,24)
	...
	#for n_steps in range(1):
		...
		input_to_c_lstmD_fix = z_lstmD_fix[:, layers*2:layers*3]>>scale_shift#(32,24)

		o_lstmD_fix = uf.hard_sig(z_lstmD_fix[:, layers*3:layers*4])#(32,12)
		c_lstmD_fix = np.multiply(f_lstmD_fix, Decoder_c_0_lstm_fix) + np.multiply(i_lstmD_fix, input_to_c_lstmD_fix)#(32,24)
		h_lstmD_fix = np.multiply(o_lstmD_fix, c_lstmD_fix>>scale_shift)#(32,24)
		...
		Output_dense_fix = Output_dense_fix
		#Output_dense_fix = Output_dense_fix>>scale_shift
...
print(Outputs_fix.shape)
#print('output :',Outputs*scale)
#print("Outputs Fixed Point:",Outputs_fix)
print("min e max error float-fix ", np.amin(np.abs(Outputs-Outputs_fix*unscale2)),":",np.amax(np.abs(Outputs-Outputs_fix*unscale2)))
avg_error = np.mean(Outputs-Outputs_fix*unscale2)
print("Errore Medio floating->fixed:",avg_error)
