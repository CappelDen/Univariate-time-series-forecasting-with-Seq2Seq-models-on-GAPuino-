...
Encoder_lstm_bias_fix = np.int32(np.trunc(scale2*Encoder_lstm_bias)) #(32,22)
Encoder_lstm_kernel_fix = np.int16(np.trunc(scale*Encoder_lstm_kernel))#(32,11)
if(np.max(scale*Encoder_lstm_kernel)>pow(2,15)-1): print("Enc Kernel overflow")
Encoder_lstm_rec_ker_fix = np.int16(np.trunc(scale*Encoder_lstm_rec_ker))#(32,11)
if(np.max(scale*Encoder_lstm_rec_ker)>pow(2,15)-1): print("Enc Rec_Ker overflow")

Decoder_lstm_bias_fix = np.int32(np.trunc(scale2*Decoder_lstm_bias))#(32,22)
Decoder_lstm_kernel_fix = np.int16(np.trunc(scale*Decoder_lstm_kernel))#(32,11)
if(np.max(scale*Decoder_lstm_kernel)>pow(2,15)-1): print("Dec Kernel overflow")
Decoder_lstm_rec_ker_fix = np.int16(np.trunc(scale*Decoder_lstm_rec_ker))#(32,11)
if(np.max(scale*Decoder_lstm_rec_ker)>pow(2,15)-1): print("Dec Rec_Ker overflow")

dense_bias_fix = np.int32(np.trunc(scale2*dense_bias))#(32,22)
dense_kernel_fix = np.int16(np.trunc(scale*dense_kernel))#(32,11)

##Input Scaling

Input_seq_test_fix = np.int32(np.trunc(input_seq_test*scale))#(32,11)
...
#for n_input in range(1):
	...
	for n_steps in range(steps_in):
		z_lstm_fix = np.dot(x_fix[n_input,n_steps], Encoder_lstm_kernel_fix) + np.dot(Encoder_h_0_lstm_fix, Encoder_lstm_rec_ker_fix) + Encoder_lstm_bias_fix#(32,22)
		i_lstm_fix = uf.hard_sig(z_lstm_fix[:, 0:layers])#(32,13)
		f_lstm_fix = uf.hard_sig(z_lstm_fix[:, layers:layers*2])#(32,13)
		input_to_c_lstm_fix = z_lstm_fix[:, layers*2:layers*3]#(32,22)		
#		input_to_c_lstm_fix = (input_to_c_lstm_fix)>>12#(32,12)non fa troncamento ma arrotondamento		
		o_lstm_fix = uf.hard_sig(z_lstm_fix[:, layers*3:layers*4])#(32,13)
...
Outputs_fix = (np.array(Outputs_fix)*unscale2)
avg_error = np.mean(Outputs-Outputs_fix)
print("Outputs Fixed Point:",Outputs_fix)
#print("min e max error float-fix ", np.amin(np.abs(Outputs-Outputs_fix)),":",np.amax(np.abs(Outputs-Outputs_fix)))
