...
model = h5py.File('27_9EDU1LSTM_100OneYear.h5', 'r')
input_seq_test = np.load('Input_seq_test.npy')
output_seq_test = np.load('Output_seq_test.npy')

dense_bias = np.array((((model['model_weights'])['dense_13'])['dense_13'])['bias:0'])
dense_kernel = np.array((((model['model_weights'])['dense_13'])['dense_13'])['kernel:0'])

Encoder_lstm_bias = np.array((((model['model_weights'])['rnn_25'])['rnn_25'])['bias:0'])
Encoder_lstm_kernel = np.array((((model['model_weights'])['rnn_25'])['rnn_25'])['kernel:0'])
Encoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_25'])['rnn_25'])['recurrent_kernel:0'])

Decoder_lstm_bias = np.array((((model['model_weights'])['rnn_26'])['rnn_26'])['bias:0'])
Decoder_lstm_kernel = np.array((((model['model_weights'])['rnn_26'])['rnn_26'])['kernel:0'])
Decoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_26'])['rnn_26'])['recurrent_kernel:0'])
