...
model = h5py.File('27_27EDU1LSTM10T_250OneYear.h5', 'r')
print(list(model.keys())[0])

input_seq_test = np.load('Input_seq_test.npy')
output_seq_test = np.load('Output_seq_test.npy')

dense_bias = np.array((((model['model_weights'])['dense_2'])['dense_2_2'])['bias:0'])
dense_kernel = np.array((((model['model_weights'])['dense_2'])['dense_2_2'])['kernel:0'])

Encoder_lstm_bias = np.array((((model['model_weights'])['rnn_3'])['rnn_3_2'])['bias:0'])
Encoder_lstm_kernel = np.array((((model['model_weights'])['rnn_3'])['rnn_3_2'])['kernel:0'])
Encoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_3'])['rnn_3_2'])['recurrent_kernel:0'])

Decoder_lstm_bias = np.array((((model['model_weights'])['rnn_4'])['rnn_4_2'])['bias:0'])
Decoder_lstm_kernel = np.array((((model['model_weights'])['rnn_4'])['rnn_4_2'])['kernel:0'])
Decoder_lstm_rec_ker = np.array((((model['model_weights'])['rnn_4'])['rnn_4_2'])['recurrent_kernel:0'])
...
np.savetxt('Encoder_lstm_bias.txt', Enc_bias, delimiter=',',newline=',', fmt='%d')
np.savetxt('Encoder_lstm_kernel.txt', Enc_kernel, delimiter=',',newline=',',  fmt='%d')
np.savetxt('Encoder_lstm_rec_ker.txt', Enc_rec_ker, delimiter=',',newline=',\n',  fmt='%d')
...
np.savetxt('Decoder_lstm_bias.txt', Dec_bias, delimiter=',',newline=',',  fmt='%d')
np.savetxt('Decoder_lstm_kernel.txt', Dec_kernel, delimiter=',',newline=',',  fmt='%d')
np.savetxt('Decoder_lstm_rec_ker.txt', Dec_rec_ker, delimiter=',',newline=',\n',  fmt='%d')
